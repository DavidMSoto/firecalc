...
import { formatCurrency, formatPercentage } from '../../../../common/utils/formatting';
...
export const getTooltipContent = {
  ...
  statePension: (row: SimulationRow, index: number) => {
    const isEligibleAge = row.age >= STATE_PENSION_AGE;
    const isEligibleNI = Math.min(row.age - 18, FULL_NI_YEARS) >= MIN_NI_YEARS;
    const niYears = Math.min(row.age - 18, FULL_NI_YEARS);
    const niProportion = Math.min(1, niYears / FULL_NI_YEARS);
    const inflationMultiplier = Math.pow(1 + INFLATION_RATE, index);
    ...
    if (!isEligibleAge) {
      eligibilityStatus = `Not yet eligible (${STATE_PENSION_AGE - row.age} years until State Pension age)`;
    ...
    return `
      ...
      • Inflation Adjustment: ${formatPercentage(inflationMultiplier * 100 - 100)} over ${index} years
